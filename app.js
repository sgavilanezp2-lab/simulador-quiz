...
btnReview.onclick = () => {
  ...
  ronda.forEach((p, i) => {
    ...
    p.opciones.forEach((op, k) => {
      ...
      else if (k === userAns) cls = "option-selected"; // Usuario acertó (visualmente)
...
btnGuardar.onclick = () => { localStorage.setItem(KEY, JSON.stringify({ ronda, respuestasUsuario, idx })); alert("Guardado."); };
btnCargar.onclick = () => {
