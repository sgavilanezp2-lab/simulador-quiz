...
// Validar límite de 2 dispositivos
async function validarDispositivos(user){
  ...
  if(!snap.exists){
    await ref.set({ email: user.email, devices:[devId], lastLogin:new Date() });
    return true;
  }
  ...
  await ref.update({ devices, lastLogin:new Date() });
  return true;
}
...
btnLogin.onclick = async () => {
  try {
    ...
    if(!correosPermitidos.includes(correo)){
      ...
      registrarIntentoFallido(correo); return;
    }
